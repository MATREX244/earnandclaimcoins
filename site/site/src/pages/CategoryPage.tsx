...
import { Search, Droplet, List, Activity, ChevronUp, Home } from 'lucide-react';
...
export default function CategoryPage({ category }: CategoryPageProps) {
  ...
  const categories: { id: string; label: string; path: string; icon: React.ReactNode }[] = [
    ...
    { id: 'PASSIVE', label: 'Passive', path: '/passive', icon: <ChevronUp className="w-4 h-4" /> },
  ];
    ...
    <div className="min-h-screen flex flex-col font-sans bg-[#f8fafc]">
      ...
      <header className="sticky top-0 z-50 bg-[#2e1065] text-white py-3 shadow-xl">
        <div className="max-w-[1400px] mx-auto px-6 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex items-center gap-6 text-[14.5px] text-gray-300 font-medium">
            {categories.map((cat) => (
              <a
                key={cat.id}
                href={cat.path}
      ...
      {/* Mobile bottom nav */}
      <nav className="lg:hidden fixed bottom-0 w-full z-40 bg-[#2e1065] border-t border-purple-800/50 pb-[env(safe-area-inset-bottom)] shadow-[0_-4px_24px_rgba(0,0,0,0.2)]">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full pb-24">
        ...
        {categorySites.length > 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-[24px] border border-gray-100 shadow-sm">
            <p className="text-xl text-gray-500 font-medium">Nenhum site encontrado.</p>
            <button
              onClick={() => setSearchQuery('')}
              className="mt-4 text-purple-600 font-bold hover:text-purple-700 underline underline-offset-4"
            >
              Limpar busca
            </button>
          </div>
      ...
      <footer className="bg-[#1f0b45] text-white py-12 mt-auto">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-8 pb-8 border-b border-purple-400/20">
            ...
            <nav className="flex flex-wrap justify-center gap-x-6 gap-y-3 text-[13px] text-gray-300 font-medium">
