...
export function SiteCard({ site, isFavorite = false, onToggleFavorite }: SiteCardProps) {
  ...
  // FIX: coinTimings must be stable — computed once per mount, not on every render
  ...
  }, []); // intentionally empty: timings are decoration, stability matters more

  // Navigate using a slug derived from the site name for clean URLs
  const handleClaim = (e: React.MouseEvent) => {
    e.preventDefault();
    navigate(`/redirect/${slugify(site.name)}`);
  };
